...
class generateSubtitleFormTxt:
    ...
    def convertTimecode(stt_t):
        ...
        srt_t = str(h1) + str(h2) + ":" + str(m1) + str(m2) + ":" + str(s1) + str(s2) + "," + str(back_t)
    ...
    def writeSubtitle(sentence, start_time, end_time, file_name):
        ...
        srt_path = 'player/media/videos/test.srt'
        f = open(srt_path, 'w')
    ...
    # Load data file
    input_file = open('player/media/text/test.txt', 'r', encoding='utf-8')
    ...
    writeSubtitle(sentence, sent_start, sent_end, '0412.srt')
